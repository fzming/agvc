    ...
    public abstract class RedisCaching
    {
        ...
        public Task<T> DoAsync<T>(Func<IDatabase, Task<T>> asyncFunc)
        {
            var retryPolicy = Policy.Handle<RedisException>()
                .WaitAndRetryAsync(RetryMaxTimes, count => TimeSpan.FromSeconds(count));
        ...
        {
            var retryPolicy = Policy.Handle<RedisException>()
                .WaitAndRetry(RetryMaxTimes, count => TimeSpan.FromSeconds(count));
            var database = RedisConnectionMultiplexer.ConnectionMultiplexer.GetDatabase();
        ...
        {
            if (!val.HasValue || val.IsNull || val.IsNullOrEmpty) return default;
        ...
        {
            if (!val.HasValue || val.IsNull || val.IsNullOrEmpty) return null;

            if (type.IsValueType || type == typeof(string)) return SafeCasting.CastTo(type, val);
