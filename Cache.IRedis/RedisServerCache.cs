...
namespace Cache.IRedis
{
    ...
    {
        public RedisServerCache(IRedisConnectionMultiplexer redisRedisConnectionMultiplexer) : base(
        ...
        public string[] KeyScan(string pattern = "*", int pageSize = 10)
        {
            return DoServerSync(
                (server, database) =>
                    server.Keys(database, pattern, pageSize).Select(p => (string) p).ToArray());
        }
