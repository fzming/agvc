...
namespace CoreData.Core
{
    ...
    public abstract class MongoEntity : PrimaryAggregateEntity<string>
    {
        ...
        [BsonRepresentation(BsonType.ObjectId)] //使用 [BsonRepresentation(BsonType.ObjectId)] 进行批注，以允许将参数作为类型 string 而不是 ObjectId 结构进行传递。 Mongo 处理从 string 到 ObjectId 的转换。
