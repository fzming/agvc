...
namespace AgvcWorkFactory
{
    ...
    public class VirtualRobot : IDisposable, IVirtualRobot
    {
        ...
        public void SetMRStatus(MRStatus eMrStatus)
        {
            ...
            {
                Console.WriteLine(
                    $"{MRStatus.MRID} StatusChange,{MRStatus.IOperatorStatus.ToString()},{MRStatus.MissionStatus}");
                MRStatus = eMrStatus;
            }
        }
        ...
        ///     当前待处理任务
        /// </summary>
        private readonly Queue<IRobotTask> _tasks = new();
        ...
        private bool CheckBattery()
        {
            ...
            if (MRStatus.Battery < agvcConfig.ChargePolicy.Battery) //电量不足[可配置]
            {
                ...
                if (!Docking && MRStatus.IOperatorStatus != IOperatorStatus.Docked)
                {
                    ...
                    BeforeDockBattery = MRStatus.Battery;
                    Docking = WS.SendDockMission(MRStatus.MRID);
                    Console.WriteLine($"[DockingBattery]{MRStatus.MRID} Dock:{Docking}");
                    State = "充电中";
                }
