...
namespace AgvcRepository.Users
{
    public class DiscussMessageRepository : MongoRepository<DiscussMessage>, IDiscussMessageRepository
    {
            ...
            DiscussMessagePageQuery query)
        {
            ...
            q = query.OnlyNew
                ? q.Where(p => p.UserFlags.Any(k => k.Id == query.UserId) == false)
                : q.Where(p => p.UserFlags.Any(k => k.Id == query.UserId && k.Flag == MessageFlag.已删) == false);
        ...
        public async Task<Dictionary<string, int>> GetUnReadMessageCountAsync(string orgId, string clientId,
            string[] groups)
        {
        ...
        {
            var user = isSys
                ? await SystemUserRepository.GetAsync(clientId)
                : await AccountRepository.GetAsync(clientId) as UserAccountBase;
            if (user == null) return new UserIdentity();
        ...
        public DiscussMessageRepository(IMongoUnitOfWork unitOfWork, IAccountRepository accountRepository,
            ISystemUserRepository systemUserRepository) : base(unitOfWork)
        {
