...
namespace Utility.Extensions
{
    ...
    public static class BitmapExtension
    {
        ...
        {
            return (byte[])System.ComponentModel.TypeDescriptor.GetConverter(image).ConvertTo(image, typeof(byte[]));
        }
            ...
            int maxHeight = 0)
        {
            ...
            using (var bitmap = new Bitmap(maxWidth, maxHeight))
            {
                ...
                {
                    imgPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, imgPath);
                }
        ...
        public static string ToFile(this Bitmap bitmap, string filepath = "")
        {
            ...
            if (!Path.IsPathRooted(filepath))
            {
                filepath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, filepath.IsNullOrEmpty() ? $"temp\\{Guid.NewGuid():D}.jpg" : filepath);
            }
