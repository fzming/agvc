...
namespace Utility.Helpers
{
    ...
    public static class PathHelper
    {
        ...
        {
            if (paths.Length == 0) throw new ArgumentException("please input path");
            ...
            if (firstPath.StartsWith("HTTP", StringComparison.OrdinalIgnoreCase)) spliter = "/";

            if (!firstPath.EndsWith(spliter)) firstPath = firstPath + spliter;
            builder.Append(firstPath);

            for (var i = 1; i < paths.Length; i++)
            {
                var nextPath = paths[i];
                if (nextPath.StartsWith("/") || nextPath.StartsWith("\\")) nextPath = nextPath.Substring(1);
        ...
        public static string MapPath(string strPath)
        {
            ...
            if (strPath.StartsWith("../")) //使用父目录的相对路径转绝对路径 注意，只支持../开头
            {
                ...
                for (var i = 0; i < count; i++) rootDir = rootDir?.Parent;
            ...
            strPath = strPath.Replace("~", "");
            if (strPath.StartsWith("//")) strPath = strPath.TrimStart('/').TrimStart('/').Replace('/', '\\');
