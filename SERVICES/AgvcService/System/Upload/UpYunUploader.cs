...
namespace AgvcService.System.Upload
{
    ...
    internal class UpYunUploader : IUpYunUploader
    {
        ...
        private string Domain =>
            $"http://{(Setting.UserDomain.IsNullOrEmpty() ? Setting.ApiDomain : Setting.UserDomain)}";
        ...
        private string ParseUploadPath(string filePath)
        {
            if (FileSet.AutoMkdir == false) //不自动创建目录，将采用待上传文件一致的文件目录结构
            {
                var directory = Path.GetDirectoryName(filePath);
                var folder = Regex.Replace(directory ?? throw new InvalidOperationException(), @"\w+:", "",
        ...
        public async Task<long> GetFolderUsageAsync(string url)
        {
            ...
            long size;
            using var resp =
                await CreateWorkerAsync("GET", Setting.DL + Setting.BucketName + url + "?usage", null, headers);
            try
            {
        ...
        private FileSetting FileSet { get; }
        ...
        private async Task<HttpResponseMessage> CreateWorkerAsync(string method, string url, byte[] postData,
            Dictionary<string, object> headers)
        {
            ...
            if (FileSet.AutoMkdir) byteContent.Headers.Add("mkdir", "true");

            if (postData != null)
            {
                if (FileSet.ContentMd5 != null) byteContent.Headers.Add("Content-MD5", FileSet.ContentMd5);
                if (FileSet.FileSecret != null) byteContent.Headers.Add("Content-Secret", FileSet.FileSecret);
            }
        ...
        private string ConvertBucketPath(string path)
        {
            ...
            if (!path.StartsWith(Setting.DL)) path = Setting.DL + path;
