...
namespace AgvcService.System
{
    ...
    public class SystemUserService : AbstractService, ISystemUserService
    {
        ...
        public async Task<Result<SystemUser>> CreateSystemUserAsync(SystemUserCreateModel userCreateModel, string orgId)
        {
            ...
            var user = await FindSystemUserByLoginId(userCreateModel.LoginId);
            if (user != null) return Result<SystemUser>.Fail("账号名称已经存在");

            if (userCreateModel.Mobile.IsMobile() &&
                await SystemUserRepository.AnyAsync(p => p.Mobile == userCreateModel.Mobile))
        ...
        public async Task<Result<SystemUser>> UpdateSystemUserAsync(SystemUserUpdateModel userUpdateModel)
        {
            ...
            if (!validate.Success) return Result<SystemUser>.Fail(validate.Error);
            var user = await SystemUserRepository.GetAsync(userUpdateModel.UserId);
            if (user == null) return Result<SystemUser>.Fail("待修改的账户不存在");
            //检查是否更换了账户登录ID
            if (user.LoginId != userUpdateModel.LoginId)
                if (await SystemUserRepository.AnyAsync(p => p.LoginId == userUpdateModel.LoginId))
            ...
            if (userUpdateModel.LoginPwd.IsNullOrEmpty()) user.LoginPwd = oldPwd; //密码为空时，不修改密码
        ...
        public async Task<Result<SystemUser>> LoginAsync(SystemUserLoginModel loginModel)
        {
            ...
            if (user.OrgId.IsNotNullOrEmpty())
            {
                ...
                if (org == null) return Result<SystemUser>.Fail("当前尚未确定机构,不支持登录");
            }
        ...
        public async Task<bool> ChangeUserPasswordAsync(string clientId, ChangePasswordModel model)
        {
            ...
            if (model.NewPwd.Length < 6) throw new Exception("新密码至少需6位");
