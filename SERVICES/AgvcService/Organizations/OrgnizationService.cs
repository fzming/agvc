...
namespace AgvcService.Organizations
{
    public class OrgnizationService : AbstractService, IOrgnizationService
    {
        ...
        {
            return this.OrganizationRepository.FindAsync(p => p.ParentOrgId == paOrgId);
        }
        /// <inheritdoc/>
        public async Task<Organization> CreateOrgAsync(CreateOrgModel model, string paOrgId)
        {
            ...
            {
                if (paOrgId.IsNullOrEmpty())
                {
                    throw new Exception("非系统型机构需传入父机构ID");
                }

                if (model.Prefix.IsNullOrEmpty())
                {
                    throw new Exception("非系统型机构需传入机构简称");
                }
                if (model.Prefix.Length > 4)
                {
                    throw new Exception("机构简称不能超过4位");
                }
                ...
                if (model.Modules.AnyNullable() == false)
                {
                    throw new Exception("非系统型机构至少需选择一个功能模块");
                }
                //创建货代用户
        ...
        public async Task<bool> UpdateOrgAsync(UpdateOrgModel model)
        {
            ...
            {
                if (org.Prefix.IsNullOrEmpty())
                {
                    throw new Exception("非系统型机构需传入机构简称");
                }

                if (org.Prefix.Length > 4)
                {
                    throw new Exception("机构简称不能超过4位");
                }
                if (DomainBlacklists.Contains(model.Prefix, new IgnoreCaseComparer()))
                ...
                if (org.Modules.AnyNullable() == false)
                {
                    throw new Exception("非系统型机构至少需选择一个功能模块");
                }
            }
        ...
        {
            return this.OrganizationRepository.GroupAllIdsAsync();
        }
        /// <summary>
        /// 更新模块
        /// </summary>
        ...
        {
            return this.OrganizationRepository.UpdateModulesAsync(modules);
        }
