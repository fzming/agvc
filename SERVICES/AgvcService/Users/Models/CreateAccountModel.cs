...
namespace AgvcService.Users.Models
{
    ...
    public class CreateAccountModel : AccountModel
    {
        ...
        public override Result<bool> Validate()
        {
            if (!ForbiddenLogin)
            {
                if (LoginId.IsNullOrEmpty()) return Result<bool>.Fail("账号必须传递");
                if (LoginPwd.IsNullOrEmpty()) return Result<bool>.Fail("密码必须传递");
            }

            if (RoleId.IsNullOrEmpty()) return Result<bool>.Fail("账户的角色必须传递");
            if (Nick.IsNullOrEmpty()) return Result<bool>.Fail("姓名必须传递");
            if (Nick.IsNullOrEmpty()) return Result<bool>.Fail("账户的角色必须传递");
