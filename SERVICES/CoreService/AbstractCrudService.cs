...
namespace CoreService
{
    ...
    public abstract class AbstractCrudService<T> : AbstractService, ICrudService<T> where T : AggregateRoot, new()
    {
        ...
        /// <returns></returns>
        public virtual async Task<bool> TryValidateModelAsync<TModel>(string orgId, TModel model)
            where TModel : Model, new()
        {
            ...
            if (model is IUpdateModel updateModel) modifyId = updateModel.Id;
            ...
            {
                var tasks = noRepeatPropertyInfos.Select(
                    p => GetValidResultAsync(p, p.GetValue(model), orgId, modifyId));
        ...
        {
            await TryValidateModelAsync(orgId, addModel);
        ...
        /// <returns></returns>
        public virtual async Task<T> UpdateAsync<TModel>(TModel updateModel, Func<T, bool> updateHook = null)
            where TModel : Model, IUpdateModel, new()
        {
            if (string.IsNullOrEmpty(updateModel.Id)) throw new Exception("更新时须正确传入ID");
            var oldEntity = await Repository.GetAsync(updateModel.Id); //旧版本数据库记录
            //  var orgId = (oldEntity as OEntity)?.OrgId;
            await TryValidateModelAsync(string.Empty, updateModel);
            ...
            if (invoked)
            {
                ...
                if (updated) await HookAfterUpdateActionAsync(oldEntity, entity, OnAfterUpdate, OnAfterUpdateAsync);
            }
        ...
        public virtual async Task<bool> DeleteAsync(string id, Func<string, bool> deleteHook = null)
        {
            ...
            if (invoked)
            {
                ...
                if (deleted) await HookAfterActionAsync(id, OnAfterDelete, OnAfterDeleteAsync);
        ...
        private async Task<ValidResult> GetValidResultAsync<TField>(PropertyInfo property, TField value, string orgId,
            string modifyId)
        {
            var nrp = property.GetCustomAttribute<NoRepeatAttribute>();
            var repeat =
                await Repository.IsFieldRepeatAsync(property.Name, value, nrp.Global ? string.Empty : orgId, modifyId);
        ...
        /// <returns></returns>
        private async Task<bool> HookBeforeActionAsync<TParam>(TParam param, Func<TParam, bool> hook,
            Func<TParam, bool> onBeforeHook, Func<TParam, Task<bool>> onBeforeHookAsync)
        {
        ...
        private async Task HookAfterUpdateActionAsync<TParam>(TParam oldEntity, TParam newEntity,
            Action<TParam, TParam> onAfterHook,
